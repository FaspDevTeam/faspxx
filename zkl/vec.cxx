...
#if ASSIGN_BY_INDEX
    for (int j = 0; j < N; j++)
        vec[j] = rand() / (DBL) (end_value - begin_value) + begin_value;
#endif
#if ASSIGN_BY_PUSHBACK
    for(int j=0;j<N;j++)
        vec.push_back(rand() / (DBL) (end_value - begin_value) + begin_value);
#endif
#if ASSIGN_BY_ITERATOR
    for(std::vector<DBL>::iterator iter=vec.begin();iter!=vec.end();iter++)
        *iter=rand() / (DBL) (end_value - begin_value) + begin_value;
#endif
}
...
FaspErrorType VEC::min(DBL *min){
    ...
    if(len==0){
        min= nullptr;
        //! errortype 1 marks the empty VEC object
        return 1;
    }
    *min=vec[0];
    for(int j=0;j<len;j++){
...
FaspErrorType VEC::norm2(DBL *norm2){
    int len=vec.size();
    if(len==0)
        return 0; //! errortype 1 marks the empty VEC
...
FaspErrorType VEC::add(std::vector<DBL> vector1, DBL a, DBL b){
    ...
    if(len!=len1)
        return 2; //! 3 marks the mismatch of vectors' lengths
