...
class Vector{
    ...
    public:
        ...
        //! set the row number on the class object
        FaspErrorCode SetRow(int row){
            if(row<=0){
                //! 1 is illegal error
                return 1;
            }
        ...
        //! set a value on class object
        FaspErrorCode SetValue(int index,double value){
        ...
        //! this->val = vector1 + vector2
        FaspErrorCode VecAddVec(Vector a,Vector b){
            if(a.row!=b.row || row!=a.row){
                //! 2 is the array length mismatch error
                return 2;
            }
            int j;
