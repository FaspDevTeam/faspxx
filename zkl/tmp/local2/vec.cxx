...
FaspErrorType VEC::GetSize(INT &size) const {
    size = this->size;
    return 0;
}
...
FaspErrorType VEC::Add2(const DBL a, const VEC vec1_obj, const DBL b, const VEC vec2_obj){
    if(vec1_obj.size!=vec2_obj.size)
        return 3; //! 3 marks the mismatch of vectors' lengths

    std::vector<DBL> tmp(vec1_obj.size);

    for(int j=0;j<vec_x.size;j++)
        tmp[j]=a*vec_x[j]+b*vec_y[j];

    (this->vec).operator=(tmp);
    this->size=vec_x;
...
FaspErrorType VEC::Dot(const VEC vec, DBL &dot) const {
