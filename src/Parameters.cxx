...
void Parameters::AddParam(double* _ptr, const char* _name, const char* _help)
{
    params.push_back(Param(_DOUBLE, _ptr, _name, _help));
}
    ...
    for (int i=1; i<argc; i++)
    {
        ...
        for (int j=0; j<params.size(); ++j)
        {
            if (strcmp(argv[i], params[j].param_name) == 0)
            {
                ...
                {
                    case _BOOL:
                        if (!strcmp(argv[i+1], "true") || !strcmp(argv[i+1], "True") || !strcmp(argv[i+1], "TRUE"))
                            ...
                            *((bool*)(params[j].param_ptr)) = (bool)(std::stoi(argv[i+1]));
                        break;
                    case _INT:
                        *((int*)(params[j].param_ptr)) = std::stoi(argv[i+1]);
                        break;
                    case _DOUBLE:
                        *((double*)(params[j].param_ptr)) = std::stod(argv[i+1]);
                        break;
                    case _STRING:
...
void Parameters::PrintParams(std::ostream &out) const
{
    ...
    for (int i=0; i<params.size(); ++i)
    {
        ...
        {
            case _BOOL:
                out << std::boolalpha << *((bool*)(params[i].param_ptr)) << std::resetiosflags(out.flags());
                break;
            case _INT:
...
void Parameters::PrintHelp(std::ostream &out) const
{
    ...
    for (int i=0; i<params.size(); ++i)
    {
        ...
        {
            case _BOOL:
                out << std::boolalpha << *(bool*)(params[i].param_ptr) << std::setiosflags(out.flags());
                break;
            case _INT:
                out << *(int*)(params[i].param_ptr);
                break;
