...
TEST(VEC_SetValue, SetValue)
{
    ...
    for (USI i = 0; i < v10.GetSize(); i++) EXPECT_EQ(v10[i], 3.14);

    v11.SetValues(v2);
    for (USI i = 0; i < v11.GetSize(); i++) EXPECT_EQ(v11[i], v2[i]);

    v12.SetValues(4, p);
    for (USI i = 0; i < v12.GetSize(); i++) EXPECT_EQ(v12[i], p[i]);
}

TEST(VEC_Get, GetValue)
{
    for (USI i = 0; i < v6.GetSize(); i++) EXPECT_EQ(v6.GetValue(i), p[i]);
}
...
TEST(VEC_Copy, Copy)
{
    VEC v;
    v6.CopyTo(v);
    for (USI i = 0; i < v6.GetSize(); i++) EXPECT_EQ(v[i], v6[i]);
}
...
TEST(VEC_Shift, Shift)
{
    DBL shift = 3.14159;
    v6.Shift(shift);
    for (USI i = 0; i < v6.GetSize(); i++) EXPECT_EQ(v6[i], p[i] + shift);
}
